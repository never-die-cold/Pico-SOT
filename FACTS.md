...
3. Python 3.12 + numpy / matplotlib（pandas 用于 `table.txt` 后处理）；依赖见 `requirements.txt`。【约定】
...
| Keff | 论文未给（正文仅给 Ha≈1 T，Ms 在 SI） | 现行脚本取 6.5×10⁵ J/m³ = ½·Ms·Ha_T（Ha 按 B=1 T 计） |
...
1. **SOT 接入**（mumax3 无原生 SOT）：Slonczewski 模块等效，内核只读 `J` 的 z 分量。【已核实：对照 `cuda/slonczewski2.cu` + 数值自检】
   ...
   ```
2. **极性约定**：`J_z = -I_sign*Jp*脉冲(t)`（macrospin）；实测 `sign(mz_final) = -sign(Hx*I)`（现行批量 run 均为 +Mz 初态；"与初始态无关"为论文结论，本仓库未跑负初态）；Hx = 0 时不翻转。【已核实】
...
4. **四象限极性**（fig3 默认：Jpk = 1.2e13、dTpk = 400 K、Tmax = 680 K）：q1 (Hx+,I+) → −0.956；q2 (Hx−,I+) → +0.987；q3 (Hx+,I−) → +0.987；q4 (Hx−,I−) → −0.956；末态为均匀单畴（无畴壁/成核，近相干翻转；仅开边界的边缘列被钉扎）。
...
8. **C0 Heating = 0**（6/8/10e12）：三档均不翻（T 恒 300 K），峰值瞬态下拉仅 −8.1%/−12.4%/−17.6%（相对弛豫平衡态 mz=0.988，≈33 ps；表首行 t=0 为未弛豫态 mz=1，勿用作基准）→ 6–10e12 的确定性翻转完全依赖焦耳热。
9. **C1 相图边界**：9e12/300 K 不翻（末态 +0.933）；8e12/375 K 翻 66.5 ps（654 K）；7e12/450 K 翻 61.0 ps（54.1 pJ）；6e12/600 K 翻 80.5 ps（39.7 pJ，Tmax=867 K > Tc）。边界：Tmax = 583 K 线上 Jc ∈ (9,10]e12；Jp = 8e12 竖线上 dTc ∈ (300,375] K。
10. **b0_8e12**（J_ref = 6e12，dT_ref = 300 K → Tmax = 804 K）：54.5 ps 翻转。
11. **Fig. 4 动力学**（a_f4_* / f4_*）：进动周期 ≈ 37 ps（论文 Ha≈1 T 对应 ~40 ps）；平行组下冲 −4.8% @15.8 ps；反平行组在绘制的 ΔMz/Ms 上**无正上冲**（最低 −3.5% @31.8 ps），原始 mz 会从 0.987 升到 0.999，但被 Ms(T) 下降掩盖；图/表 t=0 的 +1.3% 尖峰是首行未弛豫态（mz=1）伪影，所有 Hx≠0 曲线都有，并非反平行特征。Hx = 0 时 ±I 曲线重合且无振荡；400 ps 后恢复至 ≈ −0.25%；echo = 0.3、ted = 24 ps 在 T(t) 上产生次级峰（主脉冲 t0=5 ps，峰位 ~29–31 ps）。`dyn_Jp1e12_Ip`（RunDynamics=1、3.7 ps、1e12、Tmax ≈ 308 K）：末态 +0.9879，不翻转。
...
2. `table.txt` 列（macrospin）：`t`(s), `mx`, `my`, `mz`, `E_total`, `J`(A/m²), `T`(K)；fig3/fig4 脚本以 `Ms`(A/m) 列替代 `E_total`。
3. `summary.csv` 列：`tag, time, t_end_ps, mz_final, switched, Tmax_K, Hx, Isign, t_cross_ps, recov_50ps, Jp, J_ref, dT_ref, Pol, EpsilonPrime, KuExp, echo, ted, Heating, RunDynamics, Hx_mT`。其中 `switched = InitMz·mz_final < −0.5`（与脚本判定一致；部分翻转记 0）。
...
3. 本模型最快 39 ps（vs 论文模型 16 ps）；能翻转的案例能量 39.7–441 pJ（7e12/dT450 = 54.1 pJ；6e12/dT600 = 39.7 pJ，但 Tmax = 867 K > Tc，接近论文排除的 HAMR 情形），除 6e12/dT600 外均超论文 < 50 pJ 预算；Jp = 6e12 在 dT300 档不翻转。
4. 论文准静态 Jc(Hx)（100 µs，依赖热激活）不适合直接长时间 LLG 复现。
5. 传输线反射（echo）用叠加延迟脉冲近似。
6. 论文实际为 20×4 µm 磁条，仿真只取 5×4 µm 过流区。
7. 【历史】交接文档中的旧参数（Keff ≈ 8.2e5 且 Ku1 ≈ 1.88e6、脚本 `SOT_ps_switching.mx3`）已废弃，勿回退；现行脚本用 Ku1 ≈ 1.71e6。该文档已在文首与 §2–§4 加过时标注。
