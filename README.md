...
**Status (2026-09-16):** the material and thermal model follows the paper
item by item (see
[`docs/paper_replica.md`](docs/paper_replica.md)): Ms(300 K)=1.0×10⁶ A/m, B<sub>K</sub>=0.8 T,
alpha=0.23, Ms(T)=Ms(0)[1−(T/Tc)^1.7], Kz(T)∝Ms³, and heating as the 0D equivalent
channel of the paper's heat-diffusion model (6×10¹² / 6 ps → peak +50.4 K, τ=245 ps).

## Results at a glance

| ![](docs/figures/heat_model.png) | ![](docs/figures/phase_map.png) |
|---|---|
| **Thermal model**: 1D FD vs the 0D .mx3 channel (`heat_model.py`) | **Threshold map**: mz<sub>final</sub> vs Jp, 4 series (`plot_phase.py`) |
...
* **Pure SOT switches without Joule heating**: threshold 20×10¹² at θ<sub>DL</sub>=0.2
  (~13.5×10¹² at θ<sub>DL</sub>=0.3); with the paper's heat channel it drops to 10×10¹² / ~8.5×10¹² →
  **heating lowers the threshold current by ~2× (energy by ~2.3–4×)**, matching the
  ratio stated in the paper (pure LLG 9×10¹² → with heating 6×10¹², ~2× energy).
...
| `ThetaDL` | θ<sub>DL</sub> (0.20 main-text fit; 0.30 for strong-current simulations) |
| `Hx_mT` / `I_sign` / `InitMz` | in-plane bias field / current polarity / initial state |
| `Heating` | 1 = heat channel (Ms/Kz follow T); 0 = frozen 300 K (pure LLG) |
...
the scripts; see `docs/paper_replica.md`. `heat_model.py` re-derives them and plots.

## 4. Two key conventions (pitfalls, see notes/error.md)

1. **Anisotropy**: the current scripts set `Ku1 = Kz(T)` directly (the paper's Kz);
   together with mumax3's thin-film demag this realizes the paper's field
   `H_z=(2Kz/μ0Ms−Ms)m_z` exactly. Do **not** use the
...
## 5. Verified results (2026-09-16, paper parameters)
...
**Mechanism decomposition** (heating on, θ=0.2):
...
| θ≈0 (thermal-anisotropy torque only) | switches at 12×10¹² (122.5 ps), 14×10¹² (80.2 ps) — paper |
...
`runs/summary.csv`. **`runs/` is kept local:
the run data stays on disk, and the deliverable carries code, docs and the
final figures (`docs/figures/`).** Batch scan example:
...
2. ~~Time-resolved dynamics (Fig. 4a,b)~~ (done, paper parameters): `docs/figures/fig4_full.png`.
3. ~~Single-pulse switching (Fig. 3)~~ (done): four quadrants + device threshold (9, 10]×10¹².
4. ~~Thermal-model calibration~~ (done): `heat_model.py` per the paper's heat equation; 0D channel error 5.1%.
...
* Pulse waveform assumed sech² (the paper does not specify) → absolute thresholds ~1.5×
  above the paper's values (ratios match);
...
If you use this work, please cite the original paper and mumax3:
...
* Third-party literature remains with its authors/publishers and is not redistributed.
