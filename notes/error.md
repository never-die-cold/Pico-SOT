...
- **C0 Heating=0 对照**：与 `b0_6e12`/`b0_8e12`/`b1_Jp10_dT300` 同参数、仅 `Heating=0`。Jp=6/8/10e12 三档均不翻转（末态 mz≈+0.988、T 恒 300 K），最大瞬态下拉 −8.1%/−12.4%/−17.6%（相对弛豫平衡态 0.988，≈33 ps；表首行 t=0 为未弛豫态，勿用作基准）→ 6–10e12 的确定性翻转完全依赖焦耳热致热各向异性力矩。目录 `runs/c0_noh_*`，对照图 `runs/heating_on_off.png`、`runs/mechanism_compare.png`（c0/b0、b2、b3 三面板）。
...
- **F 能量核算（`energy_check.py`，ρ=81 μΩ·cm，V=5×4 µm²×15 nm）**：Jp=6e12 → **39.7 pJ**（与论文 40 pJ 口径一致）；能翻转的案例能量 54.1 pJ（7e12/450 K，Tmax=725 K）、39.7 pJ（6e12/600 K，Tmax=867 K>Tc，接近论文排除的 HAMR 情形）、70.6 pJ（8e12/450 K）、158.9 pJ（1.2e13/450 K）、441 pJ（2e13/450 K，最快 39 ps）。即本模型已能靠 6e12（论文上限）+ 强加热翻转并落回 <50 pJ 预算；不依赖过 Tc 加热的最低能量档为 7e12/450 K 的 54.1 pJ。
