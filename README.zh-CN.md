...
**当前状态（2026-09-16）**：材料/热模型已逐项对齐论文补充材料（SI Note 3 + Table S1，
见 [`docs/si_replica.md`](docs/si_replica.md)）：Ms(300 K)=1.0e6 A/m、B_K=0.8 T、α=0.23、
Ms(T)=Ms(0)[1−(T/Tc)^1.7]、Kz(T)∝Ms³、加热为 SI 热扩散方程的 0D 等效通道
（6e12/6 ps → 峰值 +50.4 K、τ=245 ps）。旧版（猜测参数 dT_ref 旋钮）结果以
...
|---|---|
| **热模型标定**：SI 1D 热扩散 FD vs .mx3 的 0D 通道（`heat_model.py`） | **阈值图**：mz_final vs Jp（4 系列，`plot_phase.py`） |
...
* **纯 SOT 无加热可以翻转**：θ=0.2 时阈值 20e12（θ=0.3 时 ~13.5e12）；
  开 SI 热通道后降到 10e12 / ~8.5e12 → **加热把阈值电流降低 ~2×（能量 ~2.3–4×）**，
  与论文 SI Fig. 3/4 的"纯 LLG 9e12 → 含热 6e12（能量 ~2×）"比值一致。
* **极性规则** `sign(mz_final) = −sign(Hx·I)`（全器件四象限验证）；`Hx=0` 永不翻转
  （对称性破缺必需，宏自旋与全器件均验证）。
* **机制分解**：Kz(T) 冻结 → 14e12 仍不翻（必要通道）；只冻结 Ms(T)（Kz 塌缩保留）
  → 接近翻转；θ≈0 纯热各向异性力矩 → 12e12 起翻但更慢（对应 SI Fig. 5）。
* **脉宽窗口**：10e12 无加热时 12 ps 不翻、15 ps 起翻 → FMR 半周期条件复现。
* **能耗**：模型阈值（10e12/θ=0.2）→ 110 pJ，高于论文 <50 pJ 预算口径（6e12）；
  绝对阈值整体比 SI 高 ~1.5×，最可能是 SI 未给脉冲波形/反射序列。
* **局限**：Jp≳1.9e13 时模型温升超过 Tc（HAMR 型，论文实验已排除）；mumax3 的
  Langevin 噪声为固定种子，无法做 P_sw(Jp) 统计。

完整定量记录见 [`docs/RESULTS.md`](docs/RESULTS.md)（§0 = SI 版，§1–8 = legacy）。
...
docs/
├─ RESULTS.md                          # 完整英文实验记录（§0 SI 版 / §1-8 legacy）
├─ ROADMAP.md                          # 完善计划与待办（中文）
└─ si_replica.md                       # SI ↔ mumax3 逐项映射与热模型标定（中文）
papers/                                # 文献（PDF 因版权不随仓库分发）
notes/
└─ error.md                            # mumax3 复刻踩坑记录（已发布；notes/NE_2020SI 仅本地）
...
| `ThetaDL` | θ_DL（0.20 主文拟合；0.30 = SI Fig.3–5 口径） |
| `Hx_mT` / `I_sign` / `InitMz` | 面内辅助场 / 电流极性 / 初态 |
| `Heating` | 1 = SI 热通道（Ms/Kz 随 T）；0 = 冻结 300 K（纯 LLG） |
| `ScaleMs` / `ScaleKz` | 分别冻结 Ms(T) / Kz(T)（机制分解） |
| `Noise` | 1 = 开 Langevin 噪声（Temp=T(t)；固定种子） |
| `t_free` | 脉冲后自由演化时间 |

热学常数（ρ、C、Λ、G、d_stack）与 Ms(T)/Kz(T) 律写死在脚本内，来源见
`docs/si_replica.md`；`heat_model.py` 可独立复算并出图。

## 4. 两个关键约定（踩坑记录，详见 notes/error.md）

1. **各向异性**：现行脚本直接 `Ku1 = Kz(T)`（SI 的 Kz），配合 mumax3 的薄膜退磁
   恰好实现 SI 的 `H_z=(2Kz/μ0Ms−Ms)m_z`。**不要**写成 legacy 的
...
论文结论，本仓库未单独跑负初态）；`Hx = 0` 时不翻转。

## 5. 已核实结果（2026-09-16，SI 参数）

**阈值（宏自旋 64×64、6 ps sech²、Hx=160 mT；Tmax=300+50.4(Jp/6e12)² K）**

| 系列 | 阈值 Jc | 说明 |
|---|---|---|
| θ=0.2，SI 加热 | (9,10]e12 | Tmax (411,438] K；10e12 起翻（68.3 ps） |
| θ=0.2，无加热 | 20e12（19e12 部分 −0.47） | **纯 SOT 可翻** |
| θ=0.3，SI 加热 | (8,9]e12 | |
| θ=0.3，无加热 | (12,14]e12（13e12 部分） | |

加热后 15–16e12 出现非单调"不翻窗口"（进动相位效应），18/20e12 恢复翻转。

**机制分解**（SI 加热，θ=0.2）：

| 对照 | 结果 |
|---|---|
| `ScaleKz=0`（Kz 冻结） | 10/12/14e12 全不翻 → Kz(T) 塌缩必要 |
| `ScaleMs=0`（只留 Kz 塌缩） | 10e12 +0.45、12e12 −0.46（接近翻转） |
| θ≈0（纯热各向异性力矩） | 12e12 翻 122.5 ps、14e12 翻 80.2 ps（SI Fig.5） |
| `Hx_mT=0` | 任何电流都不翻 |

**全器件（5×4 µm，Jpk=1.2e13，Tmax=497 K）**：四象限极性 ✓、阈值 (9,10]e12
与宏自旋一致、`Hx=0`（12e12）不翻。

**Fig.4 动力学**：平行组下冲 −5.9% @29 ps、反平行 −1.9%、Hx=0 无振荡、
周期 ≈44 ps、ΔT 峰值 +13.9 K（热模型预测 13.8 K）。

**能耗**：6e12 → 39.7 pJ（论文口径）；10e12 → 110 pJ；20e12 → 441 pJ。
...
* 均匀 Ku 模型，无成核/畴壁与随机性统计（P_sw 未做）；
* Jp≳1.9e13 时 Tmax>Tc（HAMR 型，论文实验排除），解读需谨慎；
* 传输线反射（echo）以叠加延迟脉冲近似；
* 论文的准静态 `Jc(Hx)`（100 µs 脉冲）依赖热激活，不适合直接长时间 LLG 复现；
* legacy 结果（猜测参数）仅作对照保留，勿与 SI 版混用。

## 9. 引用

若使用本仓库，请引用原论文与 mumax3（`CITATION.cff` 已收录，GitHub
